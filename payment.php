...
<script>
    ...
    document.getElementById('paymentForm').onsubmit = function(e) {
        ...
        let val = cardInput.value.replace(/\s+/g, '');
        if (!validateCard(val)) {
            cardInput.style.borderColor = "#dc2626";
            Swal.fire('خطأ', 'رقم البطاقة غير صالح.', 'error');
            return;
        }
        ...
        const interval = setInterval(() => {
            ...
            .then(data => {
                if (data.status === 'accept') {
                    ...
                    Swal.fire('فشل الدفع', 'يرجى التأكد من معلومات البطاقة أو استخدام وسيلة أخرى.', 'error');
                }
            });
        }, 3000);
    }
